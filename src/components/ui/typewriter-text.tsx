...
// See: https://www.shadcn.io/text/typing-text#why-most-typing-effects-feel-mechanical
    ...
    const executeTypingAnimation = () => {
      if (isDeleting) {
        if (displayedText === '') {
          ...
          timeout = setTimeout(() => {return}, pauseDuration); // Return to fix lint error
        } else {
          timeout = setTimeout(() => {
    ...
    showCursor && (
      <span
        ...
        className={`inline-block opacity-100 ${shouldHideCursor ? 'hidden' : ''} ${
          cursorCharacter === '|' 
            // Height is equal to height of character 'd'
            // Increase width to make the cursor look more digital
            ? `h-[2rem] w-[0.25rem] translate-y-1 bg-foreground ${cursorClassName}` 
